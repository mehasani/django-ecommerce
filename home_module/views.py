...
from product_module.serializers import LatestProductSerializer
...
class HomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        categories = list(ProductCategory.objects.annotate(products_count=Count('products_category')).filter(is_active=True,is_delete=False,products_category__gt=0)[:6])
        categories_products = []
...
class LatestProductsApiView(generics.ListAPIView):
    queryset=Product.objects.filter(is_active=True, is_delete=False).order_by('-id')[:4]
